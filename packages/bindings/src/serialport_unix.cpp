...
void EIO_Set(uv_work_t* req) {
  ...
  if (err == -1) {
    snprintf(data->errorString, sizeof(data->errorString), "Error: %s, cannot set low latency mode", strerror(errno));
    return;
  } else if(err == -2) {
    snprintf(data->errorString, sizeof(data->errorString), "Error: %s, cannot set low latency mode", strerror(errno));
    return;
  }
  // printf("Finished error codes");
  #endif
  // printf("Finished all latency mode setting");
